...
contract testing1234 is ERC721Enumerable, ReentrancyGuard, Ownable {
    using Strings for uint256;
    ...
    function generateHTMLandSVG(address _address) internal pure returns (string memory html_main, string memory svg_main) {
        (string memory color1, string memory color2, string memory color3) = getColors(_address);

        svg_main = string(abi.encodePacked('<svg xmlns="http://www.w3.org/2000/svg" width="350" height="350"><circle cx="75" cy="150" r="40" stroke="black" stroke-width="2" style="fill:#', color1,';" /><circle cx="175" cy="150" r="40" stroke="black" stroke-width="2" style="fill:#', color2,';" /><circle cx="275" cy="150" r="40" stroke="black" stroke-width="2" style="fill:#', color3,';" /><text x="31" y="240" style="font-size:23px; font-family:Arial" >Open to see the the full NFT</text></svg> '));
        html_main = string(abi.encodePacked('<!DOCTYPE html><html lang="en"><head></head><body><script type="importmap">{"imports": {"three": "https://cdn.jsdelivr.net/npm/three@0.142.0/build/three.module.js"}} </script><script type="module">import * as THREE from "three";import { OrbitControls } from "https://cdn.jsdelivr.net/npm/three@0.142.0/examples/jsm/controls/OrbitControls.js";import { RectAreaLightHelper } from "https://cdn.jsdelivr.net/npm/three@0.142.0/examples/jsm/helpers/RectAreaLightHelper.js";import { RectAreaLightUniformsLib } from "https://cdn.jsdelivr.net/npm/three@0.142.0/examples/jsm/lights/RectAreaLightUniformsLib.js";let renderer, scene, camera;init();function init() {renderer = new THREE.WebGLRenderer( { antialias: true } );renderer.setPixelRatio( window.devicePixelRatio );renderer.setSize( window.innerWidth, window.innerHeight );renderer.setAnimationLoop( animation );renderer.outputEncoding = THREE.sRGBEncoding;document.body.appendChild( renderer.domElement );camera = new THREE.PerspectiveCamera( 45, window.innerWidth / window.innerHeight, 1, 1000 );camera.position.set( 0, 5, - 15 );scene = new THREE.Scene();RectAreaLightUniformsLib.init();const rectLight1 = new THREE.RectAreaLight( 0x', color1,', 10, 4, 10 );rectLight1.position.set( - 5, 5, 5 );scene.add( rectLight1 );const rectLight2 = new THREE.RectAreaLight( 0x', color2,', 10, 4, 10 );rectLight2.position.set( 0, 5, 5 );scene.add( rectLight2 );const rectLight3 = new THREE.RectAreaLight( 0x', color3,', 10, 4, 10 );rectLight3.position.set( 5, 5, 5 );scene.add( rectLight3 );scene.add( new RectAreaLightHelper( rectLight1 ) );scene.add( new RectAreaLightHelper( rectLight2 ) );scene.add( new RectAreaLightHelper( rectLight3 ) );const geoFloor = new THREE.BoxGeometry( 2000, 0.1, 2000 );const matStdFloor = new THREE.MeshStandardMaterial( { color: 0x101010, roughness: 0.3, metalness: 0.4 } );const mshStdFloor = new THREE.Mesh( geoFloor, matStdFloor );scene.add( mshStdFloor );const geoKnot = new THREE.TorusKnotGeometry( 1.5, 0.5, 200, 16 );const matKnot = new THREE.MeshStandardMaterial( { color: 0xfffff, roughness: 0, metalness: 0 } );const meshKnot = new THREE.Mesh( geoKnot, matKnot );meshKnot.name = "meshKnot";meshKnot.position.set( 0, 4, 0 );scene.add( meshKnot );const controls = new OrbitControls( camera, renderer.domElement );controls.target.copy( meshKnot.position );controls.update();window.addEventListener( "resize", onWindowResize );}function onWindowResize() {renderer.setSize( window.innerWidth, window.innerHeight );camera.aspect = ( window.innerWidth / window.innerHeight );camera.updateProjectionMatrix();}function animation( time ) {const mesh = scene.getObjectByName( "meshKnot" );mesh.rotation.y = time / 1000;renderer.render( scene, camera );}</script></body></html>'));
                ...
                abi.encodePacked(
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                '{"name":"',
                                "testing1234 | ", uint2str(tokenId),"",
                                '", "description":"Krutaya NFT, pravda?", "image":"', imageURIsvg,'", "animation_url":"', imageURIhtml,'"}'
                            )
                        )
                    )
                )
    ...
    function mint() public {
        require(numOfMintedByWallet[msg.sender] == 0, "Dont try this again!");
        ...
        _safeMint(msg.sender, numClaimed + 1);
        numClaimed += 1;
    }
    ...
    constructor() ERC721("Ortomich 3D", "O") Ownable() {
        sale = false;
    }
}